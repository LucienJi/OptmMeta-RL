...
from envencoder.encoder import Encoder_ForwardModel
...
class MixedEnvcoder:
    def __init__(self,obs_dim,act_dim,emb_dim,parameter:Parameters) -> None:
        ...
        encoder_params = Encoder_ForwardModel.make_config_from_param(parameter)
        ### Encoder
        self.maml = MAML(Encoder_ForwardModel(obs_dim,act_dim,**encoder_params), lr = 1e-3)
        self.encoder = self.maml.encoder
        self.transition = self.maml.transition
        self.transition_target = self.transition
        ...
        self.encoder_task_parameters = self.maml.parameters()
    ...
    def compute_encoder_loss(self,obs_support_to_use,act_support_to_use,obs2_support_to_use,
        ...
        query_loss,info = learner.transition._compute_loss(obs_query_to_predict,act_query_to_predict,obs2_query_to_predict,emb_to_query)
    ...
    def save(self,path):
        self.maml.save(os.path.join(path, 'encoder_transition.pt'))
        self.moco.save(os.path.join(path, 'moco.pt'))
        torch.save(self.optm.state_dict(), os.path.join(path, 'encoder_optim.pt'))
    
    def load(self,path,**kwargs):
        self.maml.load(os.path.join(path, 'encoder_transition.pt'),**kwargs)
