...
from baselines.algorithms.ppo import PPO
...
class Runner:
    def __init__(self,
        ...
        ## Model
        #! Test
        policy_input_dim = self.env.num_obs

        value_input_dim = self.env.num_obs + self.env.num_privileged_obs
        self.policy = ActorCritic(policy_input_dim,
                                  self.env.num_actions,
                                  value_input_dim,**self.policy_cfg).to(self.device)
    ...
    def learn(self,num_learning_iterations):
        ...
        privileged_obs = self.env.get_privileged_observations() # (num_envs,num_privileged_obs)
        ...
        for it in range(self.current_learning_iteration, tot_iter):
            ...
            with torch.inference_mode(): #! More Performance Gain 
                for i in range(self.num_steps_per_env):
                    actions = self.ppo_alg.act(obs,privileged_obs) # (num_envs,num_actions)
                    next_obs,privileged_obs, rewards, dones, infos = self.env.step(actions)
                    next_obs,privileged_obs, rewards, dones = next_obs.to(self.device),privileged_obs.to(self.device), rewards.to(self.device), dones.to(self.device)
                ...
                start = stop 
                ## start update 
                self.ppo_alg.compute_returns(obs,privileged_obs)
