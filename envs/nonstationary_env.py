...
from envs.multitask.cheetah_multitask import HalfCheetahDirEnv,HalfCheetahVelEnv
from envs.multitask.ant_multitask import AntGoalEnv

register(
id='HalfCheetahDir-v1', entry_point=HalfCheetahDirEnv
)
register(
id='HalfCheetahVel-v1', entry_point=HalfCheetahVelEnv
)
register(
id='AntGoal-v1', entry_point=AntGoalEnv
)

class NonstationaryEnv(Wrapper):
    RAND_PARAMS = ['viscosity','body_mass', 'dof_damping', 'body_inertia', 'geom_friction', 'gravity', 'density',
                   'wind', 'geom_friction_1_dim', 'dof_damping_1_dim','action_mask','vel','dir','goal']
    ...
    def get_minmax_parameter(self, log_scale_limit):
        ...
        if 'goal' in self.rand_params:
            assert hasattr(self.unwrapped,'goal')
            assert hasattr(self.unwrapped,'min_goal')
            assert hasattr(self.unwrapped,'max_goal')
            min_multiplyers = bound((1,), 'low')
            max_multiplyers = bound((1,), 'high')
            min_param['goal'] = max(min_multiplyers * self.init_params['goal'],self.unwrapped.min_goal)
            max_param['goal'] = min(max_multiplyers * self.init_params['goal'],self.unwrapped.max_goal)

        if 'vel' in self.rand_params:
            assert hasattr(self.unwrapped,'vel')
            assert hasattr(self.unwrapped,'min_vel')
            assert hasattr(self.unwrapped,'max_vel')
            min_multiplyers = bound((1,), 'low')
            max_multiplyers = bound((1,), 'high')
            min_param['vel'] = min_multiplyers * self.init_params['vel']
            max_param['vel'] = max_multiplyers * self.init_params['vel']
        
        if 'dir' in self.rand_params:
            assert hasattr(self.unwrapped,'dir')
            assert hasattr(self.unwrapped,'min_dir')
            assert hasattr(self.unwrapped,'max_dir')
            min_param['dir'] = self.unwrapped.min_dir
            max_param['dir'] = self.unwrapped.max_dir
        ...
        if 'viscosity' in self.rand_params:
            min_multiplyers = bound((1,), 'low')
            max_multiplyers = bound((1,), 'high')
            min_param['viscosity'] = self.init_params['viscosity'] * min_multiplyers
            max_param['viscosity'] = self.init_params['viscosity'] * max_multiplyers
    ...
    def step(self, action):
        ...
        # print(f"NS CHANGE!########## {self.cur_step_ind, self.setted_env_changing_period}")
        if self.setted_env_params is not None and self.cur_step_ind % self.setted_env_changing_period == 0:
    ...
    def sample_tasks(self, n_tasks, dig_range=None, linspace=True):
        """
        ...
        for _ in range(n_tasks):
            ...
            if 'goal' in self.rand_params:
                vel_multiplyers = bound((1,))
                new_params['goal'] = self.init_params['goal'] * vel_multiplyers

            if 'vel' in self.rand_params:
                vel_multiplyers = bound((1,))
                new_params['vel'] = self.init_params['vel'] * vel_multiplyers

            if 'dir' in self.rand_params:
                new_params['dir'] = np.random.randint(0,2)
            ...
            # friction at the body components
            if 'geom_friction' in self.rand_params:
                geom_friction_multipliers = bound(self.model.geom_friction.shape)
    ...
    def resample_tasks(self,log_scale,n_task,linear,ns_sequence:list = None):
        self.log_scale_limit = log_scale
        self.min_param,self.max_param = self.get_minmax_parameter(log_scale)
        list_task = self.sample_tasks(n_task,linspace=linear) 
        ns_task = []
        if ns_sequence is not None:
            for idx in ns_sequence:
                if idx < n_task:
                    ns_task.append(list_task[idx])
                else:
                    print(f"{idx} out of range")
        else:
            ns_task = [list_task[random.randint(0,n_task-1)] for _ in range(15)]
        return list_task,ns_task
    ...
    def set_task(self, task):
        ...
        for param, param_val in task.items():
            if param == 'goal':
                self.unwrapped.set_goal(param_val)
                continue
            if param == 'vel':
                self.unwrapped.set_vel(param_val)
                continue
            if param == 'dir':
                self.unwrapped.set_dir(param_val)
                continue
            ...
            if param == 'gravity':
                ...
                self.unwrapped.model.opt.viscosity = float(param_val[0])
                continue
            else:
    ...
    def save_parameters(self):
        ...
        if 'viscosity' in self.rand_params:
            viscosity = self.unwrapped.model.opt.viscosity
            if viscosity - 0.0 <= 0.1:
                viscosity = 0.1
            self.init_params['viscosity'] = np.array([viscosity])
        ...
        if 'vel' in self.rand_params:
            assert hasattr(self.unwrapped,'vel')
            self.init_params['vel'] = np.array([1.0])
        if 'dir' in self.rand_params:
            assert hasattr(self.unwrapped,'dir') 
            self.init_params['dir'] = np.array([self.unwrapped.dir])
        if 'goal' in self.rand_params:
            assert hasattr(self.unwrapped,'goal')
            self.init_params['goal'] = np.array([np.pi/2.0])
...
if __name__ == '__main__':
    env = gym.make("AntGoal-v1")
    print(env.vel,env.goal)
    env = NonstationaryEnv(env,['vel','goal'])
    print(env.env_parameter_length)
