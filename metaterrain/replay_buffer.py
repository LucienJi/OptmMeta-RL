...
class Instance(object):
    def __init__(self,
                 obs,act,r,obs2 = None,done = False,value = False,
                 task_emb = None,task_emb2=None,true_task = None) -> None:
        self.obs = obs
        self.act = act
        self.r = r
        self.obs2 = obs2
...
class ReplayBuffer:
    ...
    description:
        data server
        给 off-policy 和 encoder-decoder 收集数据；
    """
    def __init__(self, max_replay_buffer_size, 
                 time_steps, observation_dim, 
                 action_dim, task_indicator_dim, data_usage_reconstruction, 
                 data_usage_sac, num_last_samples, permute_samples, encoding_mode):
