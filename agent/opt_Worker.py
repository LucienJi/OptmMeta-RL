...
from envencoder.advanced_envencoder.opt_envencoder import OptEnvencoder
from envs.nonstationary_env import NonstationaryEnv


class Opt_Worker(Base_Worker):
    def __init__(self, parameter: Parameters, env_name='Hopper-v2', seed=0, policy_type=Actor, encoder_type=OptEnvencoder,env_decoration = NonstationaryEnv, env_tasks=None, non_stationary=False, fix_env_setting=False) -> None:
    ...
    def get_action(self, state, policy:Actor, encoder:OptEnvencoder, deterministic, random, with_moco, device):
        with torch.no_grad():
            if with_moco:
                emb = encoder.moco.sample(self.task_ind,device,None)
            else:
                support = self.tracker.feed_support(device)
                ## TODO 
                emb = encoder.sample(support.obs,support.act,support.obs2,support.rew,method='argmax')
            if random:
    ...
    def set_weight(self,policy_state_dict,encoder_state_dict,world_decoder_state_dict,maml_state_dict):
        self.policy.load_state_dict(policy_state_dict)
        self.envcoder.encoder.load_state_dict(encoder_state_dict)
        self.envcoder.world_decoder.load_state_dict(world_decoder_state_dict)
        self.envcoder.maml.load_state_dict(maml_state_dict)

class Opt_Workers(Base_RemoteWorkers):
    def __init__(self, parameter, env_name, worker_num=2, seed=0, deterministic=False, use_remote=False, 
                 policy_type=Actor, encoder_type=OptEnvencoder, env_decoration=NonstationaryEnv, env_tasks=None, non_stationary=False):
        super().__init__(parameter, env_name, worker_num, seed, deterministic, use_remote, Opt_Worker, policy_type, encoder_type, env_decoration, env_tasks, non_stationary)
    ...
    def eval(self,test_envs:list,num_path,policy = None,envcoder = None):
        ...
        if self.use_remote:
            ray.get([worker.set_weight.remote(cur_policy.state_dict(),
                ...
                cur_envcoder.world_decoder.state_dict(),
                cur_envcoder.maml.state_dict()) for worker in self.workers])
